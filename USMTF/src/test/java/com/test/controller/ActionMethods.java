...
public class ActionMethods {
	...
	public WebDriver openBrowser()
	{
	   ...
	   // options.addArguments("--headless", "--window-size=1920,1200","--ignore-certificate-errors");
	    ...
	    //driver.get("https://usmtf-dev-app/cmwa/Login.aspx");
	...
	public void launchURL(String url)
	{
		try
		{
			...
			log.error("Exception occurred while launching browser URL : "+e.getMessage());
		}
	}
	...
	public boolean isElementPresent(String locatorDetails)
	{
		...
		try
		{
			...
			if(waitForElementClickable(locator, Constant.GLOBAL_MAX_TIMEOUT).isDisplayed())
			{
				flag = true;
				Constant.statusFlag = "Passed";
			} 	
	...
	public void enterInputMandatoryFiled(String locatorDetails, String testData)
	{
		try
		{
			...
			String locatorType = split[1];
			By locator = findByLocator(locatorType, locatorValue);
			WebElement ele = waitForElementClickable(locator, Constant.GLOBAL_MAX_TIMEOUT);
		   ele.clear();
	...
	public void press_enterInputMandatoryFiled(String locatorDetails)
	{
		try
		{
			String split[] = locatorDetails.split("~");
	...
	public void clearInputMandatoryFiled(String locatorDetails)
	{
		try
		{
			String split[] = locatorDetails.split("~");
	...
	public WebElement waitForElementClickable(By locator, int waitTime)
	{
		try
		{
			WebElement ele;
	...
	public void click(String locatorDetails)
	{
		try
		{
			...
			String locatorType = split[1];
			By locator = findByLocator(locatorType, locatorValue);
			waitForElementClickable(locator, Constant.GLOBAL_MAX_TIMEOUT).click();
			Constant.statusFlag = "Passed";
			log.info("Test step executed successfully : "+Thread.currentThread().getStackTrace()[1].getMethodName()+" "+locatorDetails);
	...
	public boolean getCheckBoxStatus(String locatorDetails)
	{
		boolean status;
		try
		{
	...
	public void selectDropDownData(String locator, String testData)
	{
		try
		{
			...
			{
				if(ele.getText().trim().contains(testData))
	...
	public void selectDropDown(String locator, String testData)
	{
		try
		{
			...
			String locatorValue = split[0];
			String locatorType = split[1];
			By locator1 = findByLocator(locatorType, locatorValue);			
			WebElement userData = driver.findElement(locator1);
			Select select=new Select(userData);
	...
	public void validateData(String locator, String testData)
	{
		try
		{
			String split[] = locator.split("~");
			...
			By locator1 = findByLocator(locatorType, locatorValue);			
			WebElement userData = driver.findElement(locator1);
			if(userData.getText().trim().contains(testData)) {
				Constant.statusFlag = "Passed";
			}else {
				Constant.statusFlag = "Passed";
			}
		
			log.info("Test step executed successfully : "+Thread.currentThread().getStackTrace()[1].getMethodName()+" "+locator1);
	...
	public Object generateRandomNumber(int numLength)
	{
		try
		{
			...
			if(String.valueOf(uniqueNum).length()==String.valueOf(number).length())
				...
				return generateRandomNumber(numLength);
		} catch (Exception e) {
			log.error("Failed to execute step : "+e);
		}
		return generateRandomNumber(numLength);
	...
	public void switchToFrame(int input)
	{
		try
		{
	...
	public void mouseHoverAction(String locatorDetails)
	{
		 ...
		 Actions actions = new Actions(FlowMethods.driver);
		 String split[] = locatorDetails.split("~");
			String locatorValue = split[0];
			String locatorType = split[1];
			By locator = findByLocator(locatorType, locatorValue);
		 WebElement target = FlowMethods.driver.findElement(locator);
		 actions.moveToElement(target).perform();
	}
	
	
	public void nagivateToHomeScreen() {
		try {
			refreshBrowser();
			scrollToElement("//label/span[text()='Yes']~Xpath");
